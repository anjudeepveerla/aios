...
function Sphere() {
  ...
  useFrame((state, delta) => {
    ...
    if (meshRef.current) {
      if (!isDragging.current) {
        // Slow idle rotation when not user-controlling
        ...
        // Smooth transition to target dragging rotation coordinate
        meshRef.current.rotation.x = MathUtils.lerp(meshRef.current.rotation.x, targetRotation.current.x, 0.15)
        meshRef.current.rotation.y = MathUtils.lerp(meshRef.current.rotation.y, targetRotation.current.y, 0.15)
      }
    }
  })
  ...
  const handlePointerMove = (e: any) => {
    ...
    // Scale standard mouse movements to 3D rotation radians
